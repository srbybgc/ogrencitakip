...
export default function UXFixes(){
 useEffect(()=>{
  ...
  const timer=setInterval(()=>{const map=window.__otImportBirthDates;if(!map||!Object.keys(map).length)return;try{const classes=JSON.parse(localStorage.getItem('ot-classes')||'[]');let changed=false;const next=classes.map(c=>({...c,students:(c.students||[]).map(s=>{const birth=map[String(s.studentNumber||'').trim()];if(birth&&s.birthDate!==birth){changed=true;return {...s,birthDate:birth}}return s})}));if(changed)localStorage.setItem('ot-classes',JSON.stringify(next));const still=next.some(c=>(c.students||[]).some(s=>map[String(s.studentNumber||'').trim()]));if(!still)delete window.__otImportBirthDates}catch{}}
  ,700)
  return()=>{observer.disconnect();document.removeEventListener('change',onFile,true);clearInterval(timer)}
 },[])
 return null
}
