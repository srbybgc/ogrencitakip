...
export default function ReferenceDashboard({ classes=[], schedule=[], documents=[], search='', setSearch=()=>{}, onClass=()=>{}, onClasses=()=>{}, onSchedule=()=>{}, onAddClass=()=>{}, onNewStudent=()=>{}, onDocument=()=>{} }) {
  ...
  let recentDocs=[]
  try { recentDocs = JSON.parse(localStorage.getItem('ot-documents')||'[]').slice(0,3) } catch {}
  ...
  const searchResults = useMemo(() => {
    ...
    classes.forEach(cls => {
      ...
      const teacher = cls.teacher || (cls.teacherName ? { name:cls.teacherName } : null)
      ...
      ;(cls.students||[]).forEach(student => {
        ...
        const parentText = `${student.parentName||''} ${student.parentPhone||''} ${student.secondParentName||''} ${student.secondParentPhone||''}`.toLocaleLowerCase('tr')
  ...
  const displayName = profile.displayName || user?.email || 'Profilini tamamla'
  const initials = displayName.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase() || '—'

  useEffect(() => {
    let active = true
    loadProfile(user?.uid).then(data => { if (active && Object.keys(data).length) { setProfile(data); localStorage.setItem('ot-profile', JSON.stringify(data)) } }).catch(()=>{})
    const sync = () => { try { setProfile(JSON.parse(localStorage.getItem('ot-profile') || '{}')) } catch {} }
    window.addEventListener('ot-profile-changed', sync)
    return () => { active=false; window.removeEventListener('ot-profile-changed', sync) }
  }, [user?.uid])

  return <main className="reference-dashboard">
    <header className="reference-topbar"><div className="reference-search-wrap"><form className="reference-search" onSubmit={e=>{e.preventDefault();setSearchOpen(true)}}><Search size={20}/><input value={search} onFocus={()=>setSearchOpen(true)} onChange={e=>{setSearch(e.target.value);setSearchOpen(true)}} placeholder="Sınıf, öğrenci, veli veya öğretmen ara…"/><button type="submit" aria-label="Ara"><Search size={17}/></button></form>{searchOpen && search && <div className="reference-search-results" onMouseLeave={()=>{}}>{searchResults.length ? <>{searchResults.map((r,i)=><button key={`${r.type}-${r.classId}-${r.studentId||i}`} className="reference-search-result" onClick={()=>{onClass(r.classId);setSearchOpen(false)}}><span className="reference-search-result-icon"><UserRound size={17}/></span><span><b>{r.title}</b><small><strong className="reference-search-label">{r.type}</strong> · {r.detail}</small></span><ChevronRight size={16}/></button>)}</> : <div className="reference-search-empty">Öğrenci, veli, öğretmen veya sınıf bulunamadı.</div>}</div>}</div><div className="reference-user"><button className="reference-bell"><Bell size={23}/><i/></button><span className="reference-avatar">{initials}</span><b>{displayName}</b><ChevronDown size={17}/></div></header>
    <section className="reference-hero"><div><p>İyi bir gün olsun <span>🌿</span></p><h1>{trDate(now)}</h1><small>Bugün de harika işler başaracaksın.</small></div><div className="reference-leaves"><i/><i/><i/><i/><i/></div></section>
    <div className="reference-grid"><div className="reference-main">
      <div className="reference-top-cards"><section className="ref-card lessons"><div className="ref-head"><h2><CalendarDays size={21}/> Bugünün Dersleri</h2><button onClick={onSchedule}>Tümünü Gör <ChevronRight size={16}/></button></div>{lessons.length?lessons.map((x,i)=><button className="ref-lesson" key={x.id} onClick={()=>onClass(x.classId)}><span className="ref-time"><i/>{x.start} – {x.end}</span><span className="ref-lesson-class"><span className="ref-blue-icon"><UsersRound size={22}/></span><span><b>{classes.find(c=>c.id===x.classId)?.name||'—'}</b><small>{x.lesson}</small></span></span><em>{i===0?'Şimdi':i===1?'Yaklaşıyor':''}</em></button>):<div className="ref-empty">Bugün için ders bulunmuyor.</div>}</section>
      <section className="ref-card quick"><div className="ref-head"><h2><Zap size={21}/> Hızlı İşlemler</h2></div><div className="ref-quick-grid"><button onClick={onNewStudent}><UserRound/><span>Yeni Öğrenci</span></button><button onClick={onAddClass}><UsersRound/><span>Sınıf Ekle</span></button><button onClick={onDocument}><FileText/><span>Belge Ekle</span></button><button onClick={onSchedule}><CalendarDays/><span>Ders Programı</span></button></div></section></div>
