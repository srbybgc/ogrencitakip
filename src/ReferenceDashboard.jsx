...
import { Bell, CalendarDays, ChevronDown, ChevronLeft, ChevronRight, FileText, MoreHorizontal, Plus, Search, Star, UserRound, UsersRound, X, Zap } from 'lucide-react'
...
export default function ReferenceDashboard({ classes=[], schedule=[], documents=[], search='', setSearch=()=>{}, onClass=()=>{}, onClasses=()=>{}, onSchedule=()=>{}, onAddClass=()=>{}, onNewStudent=()=>{}, onDocument=()=>{} }) {
  ...
  const searchResults = useMemo(() => {
    ...
    classes.forEach(cls => {
      ...
      ;(cls.students||[]).forEach(student => {
        ...
        if(studentName.toLocaleLowerCase('tr').includes(q)) out.push({type:'Öğrenci',title:studentName,detail:cls.name,classId:cls.id,studentId:student.id,student,studentClass:cls.name})
        if(parentText.includes(q)) out.push({type:'Veli',title:student.parentName||student.secondParentName||'Veli',detail:`${studentName} · ${cls.name}`,classId:cls.id,studentId:student.id,student,studentClass:cls.name})
      })
    })
  ...
  return <main className="reference-dashboard">
    ...
    {selectedStudent && <StudentSearchCard result={selectedStudent} onClose={()=>setSelectedStudent(null)} />}
  </main>
}

function StudentSearchCard({ result, onClose }) {
  const s=result.student||{}, name=`${s.firstName||''} ${s.lastName||''}`.trim() || result.title
  const parent=s.parentName || s.secondParentName || '—'
  const phone=s.parentPhone || s.secondParentPhone || '—'
  return <div className="student-search-backdrop" onMouseDown={onClose}><section className="student-search-card" onMouseDown={e=>e.stopPropagation()}><div className="student-search-card-head"><div><span>ÖĞRENCİ BİLGİSİ</span><h2>{name}</h2><p>{result.studentClass}</p></div><button className="icon-btn" onClick={onClose} aria-label="Kapat"><X size={18}/></button></div><div className="student-search-details"><div><small>Öğrenci No</small><b>{s.studentNumber||'—'}</b></div><div><small>Doğum Tarihi</small><b>{s.birthDate||'—'}</b></div><div><small>Veli</small><b>{parent}</b></div><div><small>Telefon</small><b>{phone}</b></div><div><small>2. Veli</small><b>{s.secondParentName||'—'}</b></div><div><small>2. Telefon</small><b>{s.secondParentPhone||'—'}</b></div></div>{(s.note||s.notes)&&<div className="student-search-note"><small>Not</small><p>{s.note||s.notes}</p></div>}</section></div>
}
