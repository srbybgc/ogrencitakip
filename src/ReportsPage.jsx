...
import { FileSpreadsheet, Printer, UsersRound, UserRound, Phone, Check, RotateCcw } from 'lucide-react'
import * as XLSX from 'xlsx'
import './reportsSettings.css'

const FIELDS=[['class','Sınıf'],['studentNumber','Öğrenci No'],['firstName','Öğrenci Adı'],['lastName','Öğrenci Soyadı'],['birthDate','Doğum Tarihi'],['parentName','Anne Adı'],['parentPhone','Anne Telefonu'],['secondParentName','Baba Adı'],['secondParentPhone','Baba Telefonu'],['address','Adres']]
const PRESETS={full:FIELDS.map(x=>x[0]),student:['class','studentNumber','firstName','lastName','birthDate'],parents:['class','firstName','lastName','parentName','parentPhone','secondParentName','secondParentPhone'],contact:['firstName','lastName','parentPhone','secondParentPhone']}
...
export default function ReportsPage({classes=[]}){
  const [classId,setClassId]=useState('all'),[selected,setSelected]=useState(PRESETS.student)
  ...
  const totalStudents=classes.reduce((n,c)=>n+(c.students||[]).length,0), parentCount=classes.reduce((n,c)=>n+(c.students||[]).filter(s=>s.parentName||s.secondParentName).length,0)
  const reportTitle=classId==='all'?'Öğrenci Bilgi Raporu':`${filtered[0]?.name||'Sınıf'} Öğrenci Raporu`, generatedAt=new Intl.DateTimeFormat('tr-TR',{dateStyle:'long',timeStyle:'short'}).format(new Date())
  const toggle=k=>setSelected(v=>v.includes(k)?v.filter(x=>x!==k):[...v,k]), preset=p=>setSelected(PRESETS[p])
  ...
  const printReport=()=>{const previousTitle=document.title;document.title=reportTitle;const restore=()=>{document.title=previousTitle;window.removeEventListener('afterprint',restore)};window.addEventListener('afterprint',restore);window.print()}
  return <main className="content reports-page">
    ...
    <section className="card report-card"><div className="report-builder-head"><div><h2>Rapor şablonu</h2><p className="muted">Sadece görmek istediğin sütunları işaretle.</p></div><div className="preset-actions"><button className="preset" onClick={()=>preset('student')}>Öğrenci Bilgileri</button><button className="preset" onClick={()=>preset('parents')}>Veli Bilgileri</button><button className="preset" onClick={()=>preset('full')}>Tümü</button><button className="preset" onClick={()=>preset('contact')}>Telefonlar</button><button className="preset reset" onClick={()=>setSelected([])}><RotateCcw size={14}/> Temizle</button></div></div>
      ...
      <div className="print-report-header"><div><h1>{reportTitle}</h1><p>{classId==='all'?'Tüm aktif sınıflar':filtered[0]?.name||'Sınıf'} · {rows.length} öğrenci</p></div><div className="print-report-meta"><b>Öğrenci Raporu</b><span>Oluşturulma: {generatedAt}</span><span>Seçili alan: {selected.length}</span></div></div>
      ...
      <div className="print-report-footer"><span>Öğrenci Takip Sistemi</span><span>Toplam {rows.length} öğrenci</span></div>
    </section><div className="print-note">“Raporu Yazdır / PDF” ile yalnızca düzenlenmiş rapor sayfası yazdırılır.</div>
  </main>
}
