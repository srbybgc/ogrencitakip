...
export default function App() {
  ...
  const importStudents = (rows) => {
    ...
    rows.forEach(row => { try { next = addStudentDomain(next, selectedClass, row.firstName, row.lastName, uid(), { studentNumber: row.studentNumber, parentPhone: row.parentPhone }); added += 1 } catch { skipped += 1 } })
...
function ClassDetail({ cls, schedule, documents, onBack, onAddStudent, onDeleteStudent, onDeleteClass, onDocument, onImportStudents, onArchive, setSchedule, setError }) {
...
function StudentImportModal({onClose,onImport}) {
  ...
  const parseText=text=>{const lines=text.replace(/\r/g,'').split('\n').map(x=>x.trim()).filter(Boolean);if(!lines.length)return [];const parse=line=>{const p=line.includes(';')?line.split(';'):line.includes('\t')?line.split('\t'):line.split(',');return p.map(x=>x.trim())};const first=parse(lines[0]).map(normalize);const headerNames=['öğrenci no','ogrenci no','öğrenci numarası','ogrenci numarasi','student number','studentnumber','ad','adı','isim','first name','firstname','soyad','soyadı','last name','lastname','anne','anne telefonu','anne numarası','anne numarasi','mother phone','baba','baba telefonu','baba numarası','baba numarasi','father phone'];const hasHeader=first.some(x=>headerNames.includes(x));const start=hasHeader?1:0;return lines.slice(start).map(line=>{const p=parse(line);if(hasHeader){const idx=name=>first.indexOf(name);const get=names=>{for(const n of names){const i=idx(n);if(i>=0)return p[i]||''}return ''};const full=get(['ad soyad','adı soyadı','isim soyisim','öğrenci','ogrenci']);let firstName=get(['ad','adı','isim','first name','firstname']),lastName=get(['soyad','soyadı','last name','lastname']);if(!firstName&&full){const parts=full.split(' ').filter(Boolean);firstName=parts.shift()||'';lastName=parts.join(' ')}return {studentNumber:get(['öğrenci no','ogrenci no','öğrenci numarası','ogrenci numarasi','student number','studentnumber']),firstName,lastName,parentPhone:get(['anne','anne telefonu','anne numarası','anne numarasi','mother phone'])}}const v=p.filter(Boolean);return {studentNumber:v[0]||'',firstName:v[1]||'',lastName:v.slice(2).join(' '),parentPhone:''}}).filter(x=>x.firstName)}
  const readFile=async f=>{setFile(f);setRows([]);setError('');setLoading(true);try{const n=f.name.toLocaleLowerCase('tr');if(n.endsWith('.txt')||n.endsWith('.csv')){const parsed=parseText(await f.text());if(!parsed.length)throw new Error('Dosyada öğrenci adı bulunamadı.');setRows(parsed);return}const wb=XLSX.read(await f.arrayBuffer(),{type:'array'}),raw=XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]],{defval:''}),norm=x=>String(x??'').trim().toLocaleLowerCase('tr'),out=[];for(const item of raw){const e=Object.entries(item).map(([k,v])=>[norm(k),String(v??'').trim()]),pick=names=>e.find(([k])=>names.includes(k))?.[1]||'';let first=pick(['ad','adı','isim','first name','firstname']),last=pick(['soyad','soyadı','last name','lastname']),full=pick(['ad soyad','adı soyadı','isim soyisim','öğrenci','ogrenci']);if(!first&&full){const p=full.split(' ').filter(Boolean);first=p.shift()||'';last=p.join(' ')}if(first)out.push({studentNumber:pick(['öğrenci no','ogrenci no','öğrenci numarası','ogrenci numarasi','student number','studentnumber']),firstName:first,lastName:last,parentPhone:pick(['anne','anne telefonu','anne numarası','anne numarasi','mother phone'])})}if(!out.length)throw new Error('Dosyada öğrenci adı bulunamadı.');setRows(out)}catch(e){setError(e instanceof Error?e.message:'Dosya okunamadı.')}finally{setLoading(false)}}
  return <Modal title="Toplu Öğrenci Aktar" onClose={onClose} wide><p className="muted">CSV/XLS/XLSX/TXT seç. CSV için Öğrenci No, Ad, Soyad, Anne Telefonu, Baba Telefonu sütunları desteklenir.</p><label className="file-picker"><Upload size={18}/><span>{file?file.name:'Dosya seç'}</span><input type="file" accept=".xlsx,.xls,.csv,.txt,text/plain,text/csv" onChange={e=>e.target.files?.[0]&&readFile(e.target.files[0])}/></label>{loading&&<div className="empty">Dosya okunuyor…</div>}{error&&<Notice message={error}/>} {!loading&&!error&&rows.length>0&&<div className="import-preview"><b>{rows.length} öğrenci bulundu</b>{rows.slice(0,10).map((r,i)=><div key={i}>{r.studentNumber?`${r.studentNumber} · `:''}{r.firstName} {r.lastName}{r.parentPhone?' · '+r.parentPhone:''}</div>)}</div>}<div className="modal-actions"><button type="button" className="secondary" onClick={onClose}>Vazgeç</button><button type="button" className="primary" disabled={!rows.length||loading} onClick={()=>onImport(rows)}>Aktar</button></div></Modal>
}
