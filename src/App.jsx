...
import { CalendarDays, ChevronLeft, ChevronRight, Clock3, FileText, FolderOpen, LayoutGrid, Menu, Plus, Search, Settings, Trash2, Upload, Users, X } from 'lucide-react'
...
export default function App() {
  ...
  const goClass = (id) => { setSelectedClass(id); setView('class') }
  const addClass = (name) => { if (!name.trim()) return; const item = { id: uid(), name: name.trim(), students: [] }; setClasses(v => [...v, item]); setModal(null); goClass(item.id) }
  const addStudent = (firstName, lastName) => { if (!firstName.trim() || selectedClass == null) return; setClasses(v => v.map(c => c.id === selectedClass ? { ...c, students: [...c.students, { id: uid(), firstName: firstName.trim(), lastName: lastName.trim() }] } : c)); setModal(null) }
  const deleteStudent = (sid) => setClasses(v => v.map(c => c.id === selectedClass ? { ...c, students: c.students.filter(s => s.id !== sid) } : c))
  const deleteClass = () => { setClasses(v => v.filter(c => c.id !== selectedClass)); setSchedule(v => v.filter(s => s.classId !== selectedClass)); setSelectedClass(null); setView('classes') }

  return <div className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => setView('home')}><span className="brand-mark">Ö</span><span><b>Öğrenci Takip</b><small>Günlük sınıf yönetimi</small></span></button>
      <nav className="desktop-nav">{[['home','Ana Sayfa'],['classes','Sınıflar'],['groups','Gruplar'],['schedule','Ders Programı'],['documents','Belgeler']].map(([id,label]) => <button key={id} className={view === id ? 'nav-active' : ''} onClick={() => setView(id)}>{label}</button>)}</nav>
      <button className="icon-btn mobile-menu" onClick={() => setModal('menu')}><Menu size={20} /></button>
    </header>

    {view === 'home' && <Home classes={classes} groups={groups} schedule={schedule} dayLessons={dayLessons} currentLesson={currentLesson} activeDay={activeDay} setActiveDay={setActiveDay} onClass={goClass} onSchedule={() => setView('schedule')} onDocuments={() => setView('documents')} onAdd={() => setModal('new')} />}
    {view === 'classes' && <Classes classes={visibleClasses} search={search} setSearch={setSearch} onClass={goClass} onAdd={() => setModal('class')} />}
    {view === 'class' && <ClassDetail cls={classes.find(c => c.id === selectedClass)} schedule={schedule} onBack={() => setView('classes')} onAddStudent={() => setModal('student')} onDeleteStudent={deleteStudent} onDeleteClass={deleteClass} />}
    {view === 'groups' && <Groups classes={classes} groups={groups} setGroups={setGroups} />}
    {view === 'schedule' && <Schedule classes={classes} schedule={schedule} setSchedule={setSchedule} />}
...
function Home({ classes, groups, dayLessons, currentLesson, activeDay, setActiveDay, onClass, onSchedule, onDocuments, onAdd }) {
  return <main className="content">
    ...
    <section className="card"><div className="section-head"><div><div className="section-title"><LayoutGrid size={18}/> Sınıflar</div><p className="muted">{classes.length} sınıf · {classes.reduce((a,c)=>a+c.students.length,0)} öğrenci</p></div><button className="icon-btn" onClick={onAdd}><Plus size={18}/></button></div>
      <div className="class-grid">{classes.map(c=><button className={currentLesson?.classId===c.id?'class-card priority':'class-card'} key={c.id} onClick={()=>onClass(c.id)}><span className="class-icon"><Users size={19}/></span><span><b>{c.name}</b><small>{c.students.length} öğrenci</small></span><ChevronRight size={17}/></button>)}</div>
    </section>
...
function ClassDetail({ cls, schedule, onBack, onAddStudent, onDeleteStudent, onDeleteClass }) { if(!cls) return null; const lessons=schedule.filter(x=>x.classId===cls.id); return <main className="content"><button className="back-btn" onClick={onBack}><ChevronLeft size={18}/> Sınıflar</button><div className="detail-head"><div><p className="eyebrow">Sınıf</p><h1>{cls.name}</h1><p className="muted">{cls.students.length} öğrenci</p></div><button className="danger-outline" onClick={onDeleteClass}><Trash2 size={16}/> Sınıfı Sil</button></div><section className="card"><div className="section-head"><div><div className="section-title"><Users size={18}/> Öğrenciler</div><p className="muted">Öğrenci listesi</p></div><button className="primary small" onClick={onAddStudent}><Plus size={16}/> Öğrenci Ekle</button></div><div className="student-list">{cls.students.map((s,i)=><div className="student-row" key={s.id}><span className="avatar">{s.firstName[0]}{s.lastName?.[0]||''}</span><span><b>{s.firstName} {s.lastName}</b><small>Öğrenci {i+1}</small></span><button className="icon-btn danger" onClick={()=>onDeleteStudent(s.id)}><Trash2 size={16}/></button></div>)}{!cls.students.length&&<div className="empty">Bu sınıfta henüz öğrenci yok.</div>}</div></section><section className="card"><div className="section-title"><Clock3 size={18}/> Ders Programı</div><div className="mini-lessons">{lessons.map(x=><div key={x.id}><b>{DAYS[x.day]}</b><span>{x.start}–{x.end}</span><small>{x.lesson}</small></div>)}{!lessons.length&&<div className="empty">Bu sınıfa henüz ders eklenmemiş.</div>}</div></section></main> }

function Groups({ classes, groups, setGroups }) { const [name,setName]=useState(''); const [selected,setSelected]=useState([]); const add=()=>{if(!name.trim())return;setGroups(v=>[...v,{id:uid(),name:name.trim(),classIds:selected}]);setName('');setSelected([])}; return <main className="content"><div className="page-head"><div><p className="eyebrow">Organizasyon</p><h1>Gruplar</h1><p className="muted">İstersen sınıfları üst gruplar altında toplayabilirsin.</p></div></div><section className="card form-card"><div className="section-title"><FolderOpen size={18}/> Yeni Grup</div><div className="inline-form"><input value={name} onChange={e=>setName(e.target.value)} placeholder="Örn. Sabah Grubu"/><button className="primary" onClick={add}><Plus size={17}/> Oluştur</button></div><div className="check-grid">{classes.map(c=><label key={c.id}><input type="checkbox" checked={selected.includes(c.id)} onChange={e=>setSelected(v=>e.target.checked?[...v,c.id]:v.filter(id=>id!==c.id))}/>{c.name}<small>{c.students.length} öğrenci</small></label>)}</div></section><div className="group-grid">{groups.map(g=><div className="group-card" key={g.id}><div className="panel-icon"><FolderOpen size={20}/></div><div><b>{g.name}</b><span>{g.classIds.map(id=>classes.find(c=>c.id===id)?.name).filter(Boolean).join(', ')||'Sınıf atanmadı'}</span></div><button className="icon-btn danger" onClick={()=>setGroups(v=>v.filter(x=>x.id!==g.id))}><Trash2 size={16}/></button></div>)}</div></main> }

function Schedule({ classes, schedule, setSchedule }) { const [day,setDay]=useState(todayIndex()); const [form,setForm]=useState({start:'08:40',end:'09:20',classId:classes[0]?.id||'',lesson:''}); const add=()=>{if(!form.classId||!form.lesson)return;setSchedule(v=>[...v,{id:uid(),day,start:form.start,end:form.end,classId:form.classId,lesson:form.lesson}]);setForm(v=>({...v,lesson:''}))}; return <main className="content"><div className="page-head"><div><p className="eyebrow">Haftalık plan</p><h1>Ders Programı</h1><p className="muted">Bugünkü ders otomatik olarak ana sayfada öne çıkar.</p></div></div><div className="day-strip">{DAYS.map((d,i)=><button key={d} className={day===i?'day-chip active':'day-chip'} onClick={()=>setDay(i)}><span>{d.slice(0,3)}</span><b>{dayDate(i).getDate()}</b></button>)}</div><section className="card form-card"><div className="section-title"><Plus size={18}/> Ders Ekle</div><div className="form-grid"><Field label="Başlangıç"><input type="time" value={form.start} onChange={e=>setForm({...form,start:e.target.value})}/></Field><Field label="Bitiş"><input type="time" value={form.end} onChange={e=>setForm({...form,end:e.target.value})}/></Field><Field label="Sınıf"><select value={form.classId} onChange={e=>setForm({...form,classId:e.target.value})}>{classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></Field><Field label="Ders"><input value={form.lesson} onChange={e=>setForm({...form,lesson:e.target.value})} placeholder="Örn. Türkçe"/></Field><button className="primary" onClick={add}>Ekle</button></div></section><section className="card"><div className="section-head"><div><div className="section-title"><Clock3 size={18}/> {DAYS[day]}</div><p className="muted">{schedule.filter(x=>x.day===day).length} ders</p></div></div><div className="schedule-manage">{schedule.filter(x=>x.day===day).sort((a,b)=>mins(a.start)-mins(b.start)).map(x=><div key={x.id}><span><b>{x.start}–{x.end}</b><small>{x.lesson} · {classes.find(c=>c.id===x.classId)?.name}</small></span><button className="icon-btn danger" onClick={()=>setSchedule(v=>v.filter(y=>y.id!==x.id))}><Trash2 size={16}/></button></div>)}{!schedule.filter(x=>x.day===day).length&&<div className="empty">Bu güne henüz ders eklenmedi.</div>}</div></section></main> }

function Documents({ classes, groups, documents, setDocuments }) { const [targetType,setTargetType]=useState('class'); const [targetId,setTargetId]=useState(classes[0]?.id||''); const upload=async e=>{const file=e.target.files?.[0];if(!file)return;let data=null;if(file.size<=2*1024*1024){data=await new Promise(r=>{const fr=new FileReader();fr.onload=()=>r(fr.result);fr.readAsDataURL(file)})};setDocuments(v=>[{id:uid(),name:file.name,size:file.size,type:file.type,targetType,targetId,data,createdAt:new Date().toISOString()},...v]);e.target.value=''}; const targetOptions=targetType==='class'?classes:groups; return <main className="content"><div className="page-head"><div><p className="eyebrow">Dosyalar</p><h1>Belgeler</h1><p className="muted">Dosyayı yüklemeden önce ilişkili kayıt türünü seç.</p></div><label className="primary upload"><Upload size={17}/> Dosya Seç<input type="file" onChange={upload}/></label></div><section className="card form-card"><div className="section-title"><FileText size={18}/> İlişkilendirme</div><div className="form-grid"><Field label="Kayıt türü"><select value={targetType} onChange={e=>{setTargetType(e.target.value);setTargetId('')}}><option value="class">Sınıf</option><option value="group">Grup</option></select></Field><Field label="Kayıt"><select value={targetId} onChange={e=>setTargetId(e.target.value)}><option value="">Seçin</option>{targetOptions.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field></div><p className="hint">Dosya seçtiğinde bu ilişkilendirme bilgisiyle kaydedilir. 2 MB'a kadar dosyalar bu prototipte tarayıcı içinde saklanabilir.</p></section><section className="card"><div className="section-title"><FolderOpen size={18}/> Yüklenenler</div><div className="doc-list">{documents.map(d=><div className="doc-row" key={d.id}><span className="doc-icon"><FileText size={18}/></span><span><b>{d.name}</b><small>{d.targetType==='class'?'Sınıf':'Grup'} · {d.targetType==='class'?classes.find(c=>c.id===d.targetId)?.name:groups.find(g=>g.id===d.targetId)?.name || 'Kayıt silinmiş'} · {(d.size/1024).toFixed(0)} KB</small></span>{d.data?<a className="download" href={d.data} download={d.name}>Aç</a>:<span className="hint">Metadata</span>}<button className="icon-btn danger" onClick={()=>setDocuments(v=>v.filter(x=>x.id!==d.id))}><Trash2 size={16}/></button></div>)}{!documents.length&&<div className="empty">Henüz belge yüklenmedi.</div>}</div></section></main> }
