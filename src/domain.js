...
const validDay = value => Number.isInteger(value) && value >= 0 && value <= 4
...
export function addLesson(schedule, lesson, id = crypto.randomUUID()) {
  ...
  if (!validDay(lesson.day) || !validTime(lesson.start) || !validTime(lesson.end) || lesson.start >= lesson.end) throw new Error('Ders günü veya saatleri geçersiz.')
...
export function checkIntegrity(classes, groups, schedule, documents) {
  ...
  const validSchedule = safeSchedule.filter(s => classIds.has(s.classId) && validDay(s.day) && validTime(s.start) && validTime(s.end) && s.start < s.end)
