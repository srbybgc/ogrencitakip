...
const writeLocal=(data,setItem)=>Object.entries(KEY_TO_COLLECTION).forEach(([key,name])=>setItem(key,JSON.stringify(Array.isArray(data?.[name])?data[name]:EMPTY[name])))
export async function startFirestoreSync(uid){if(!uid||typeof window==='undefined')return()=>{};const originalSetItem=window.localStorage.setItem.bind(window.localStorage),originalRemoveItem=window.localStorage.removeItem.bind(window.localStorage);let stopped=false;const enqueue=createSerializedQueue(async(name,items)=>{if(!stopped)await saveUserCollection(uid,name,items)});try{const remote=await loadUserData(uid),remoteHasData=Object.values(remote).some(items=>Array.isArray(items)&&items.length>0),localOwner=window.localStorage.getItem(OWNER_KEY);if(remoteHasData)writeLocal(remote,originalSetItem);else if(localOwner===uid){const local=readLocal();await Promise.all(Object.entries(local).map(([name,items])=>saveUserCollection(uid,name,items)))}else writeLocal(EMPTY,originalSetItem);originalSetItem(OWNER_KEY,uid)}catch(error){console.error('Firestore ilk veri senkronizasyonu başarısız:',error);throw error}
