...
export default function LifecycleOverlay(){
 ...
 useEffect(()=>{
  ...
  const nav=e=>{
   ...
   if(text!=='Arşiv'&&text!=='Çöp Kutusu')setOpen(null)
  }
  ...
  return()=>{window.removeEventListener('open-archive',a);window.removeEventListener('open-trash',t);window.removeEventListener('archive-selected-class',ac);document.removeEventListener('click',nav,true)}
 },[])
 useEffect(()=>{const tick=()=>{if(busy.current)return;const n=snap(),p=prev.current,t=read('ot-trash'),seen=new Set(t.map(x=>`${x.type}:${x.data?.id}`)),add=[];const put=(type,data,extra={})=>{if(!data?.id||seen.has(`${type}:${data.id}`))return;seen.add(`${type}:${data.id}`);add.push({id:uid(),type,data,...extra,deletedAt:new Date().toISOString()})};const nc=new Set(n.classes.map(x=>x.id));for(const c of p.classes)if(!nc.has(c.id)){const studentIds=new Set((c.students||[]).map(s=>s.id));put('class',c,{groups:p.groups.filter(g=>(g.classIds||[]).includes(c.id)),schedule:p.schedule.filter(s=>s.classId===c.id),documents:p.documents.filter(d=>(d.targetType==='class'&&d.targetId===c.id)||(d.targetType==='student'&&studentIds.has(d.targetId))),records:p.records.filter(r=>studentIds.has(r.studentId))})}const ns=new Set(n.classes.flatMap(c=>(c.students||[]).map(s=>`${c.id}:${s.id}`)));for(const c of p.classes)for(const st of c.students||[])if(!ns.has(`${c.id}:${st.id}`)&&nc.has(c.id))put('student',st,{classId:c.id,documents:p.documents.filter(d=>d.targetType==='student'&&d.targetId===st.id),records:p.records.filter(r=>r.studentId===st.id)});const cmp=(type,a,b)=>{const ids=new Set(b.map(x=>x.id));for(const x of a)if(!ids.has(x.id))put(type,x)};cmp('group',p.groups,n.groups);cmp('lesson',p.schedule,n.schedule);cmp('document',p.documents,n.documents);if(add.length)setTrash(v=>[...v,...add]);prev.current=n};const timer=setInterval(tick,700);return()=>clearInterval(timer)},[])
 ...
 const unarchive=id=>{write('classes',read('classes').map(c=>{if(c.id!==id)return c;const x={...c};delete x.archivedAt;return x}));setOpen('a')}
 ...
 if(!open)return null
 return <main className="content lifecycle-page" style={{position:'fixed',top:72,left:'min(345px,100%)',right:0,bottom:0,zIndex:19,overflow:'auto',background:'#f5f7fb',paddingTop:28}}>
